...
public class RobotContainer {
  ...
  private final Command driveTrainCommand = driveTrain.driveCommand(driverController1::getLeftY, ()->
          -driverController1.getRawAxis(4));

  // commands
  private final Tank tank = new Tank();
  private final Command tankCommand = tank.driveCommand(() -> driverController1.getLeftY(), ()->
          driverController1.getLeftX(), () -> driverController1.getRightY(), () -> -driverController1.getRightX());

  private void configureBindings() {
    // controller 1 - driver:
    tank.setDefaultCommand(tankCommand);
    driverController1.back().onTrue(tank.setOffSetCommand());
    driverController1.start().onTrue(tank.changeTankDriveCommand());
